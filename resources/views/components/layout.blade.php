...
<body class="h-full font-sans antialiased text-slate-900 bg-[#F8FAFC] flex flex-col md:flex-row min-h-screen selection:bg-[#FE774C] selection:text-white">
    ...
    {{-- ====================================================================
         ...
         ==================================================================== --}}
    <aside class="w-full md:w-64 bg-[#1E1E26] border-r border-[#363644] flex-shrink-0 flex flex-col z-30 md:sticky md:top-0 md:h-screen md:overflow-y-auto">
        ...
        <div class="h-18 px-6 flex items-center justify-between border-b border-[#363644]/70">
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5 group">
                ...
                <div class="flex items-center">
                    ...
                    <span class="w-1.5 h-1.5 rounded-full bg-[#FE774C] ml-1 mb-2"></span>
                </div>
            </a>
        </div>
        ...
        <div class="flex-1 py-6 px-3 flex flex-col gap-1.5 overflow-y-auto">
            ...
            <a href="{{ route('dashboard') }}" 
               class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('dashboard') ? 'bg-[#252530] text-white shadow-xs' : 'text-[#94A3B8] hover:text-white hover:bg-[#252530]/60' }}">
                ...
                @if(request()->routeIs('dashboard'))
                    <span class="ml-auto w-1.5 h-1.5 rounded-full bg-[#66A7F2]"></span>
                @endif
            </a>

            {{-- Nav: Mata Kuliah (Courses) --}}
            <a href="{{ route('courses.index') }}" 
               class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('courses.*') ? 'bg-[#252530] text-white shadow-xs' : 'text-[#94A3B8] hover:text-white hover:bg-[#252530]/60' }}">
                ...
                @if(request()->routeIs('courses.*'))
                    <span class="ml-auto w-1.5 h-1.5 rounded-full bg-[#FFC152]"></span>
                @endif
            </a>
            ...
            @if (Route::has('users.index'))
                <a href="{{ route('users.index') }}" 
                   class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('users.*') ? 'bg-[#252530] text-white shadow-xs' : 'text-[#94A3B8] hover:text-white hover:bg-[#252530]/60' }}">
                    ...
                    @if(request()->routeIs('users.*'))
                        <span class="ml-auto w-1.5 h-1.5 rounded-full bg-[#5DD299]"></span>
                    @endif
                </a>
            @endif
            ...
            <a href="{{ route('tentang') }}" 
               class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('tentang') ? 'bg-[#252530] text-white shadow-xs' : 'text-[#94A3B8] hover:text-white hover:bg-[#252530]/60' }}">
                ...
                @if(request()->routeIs('tentang'))
                    <span class="ml-auto w-1.5 h-1.5 rounded-full bg-[#FE774C]"></span>
                @endif
            </a>
        </div>
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-[#F8FAFC]">
        ...
        {{-- Top Header Bar --}}
        <header class="h-16 bg-white border-b border-slate-200/80 px-6 sm:px-8 flex items-center justify-between sticky top-0 z-20">
            ...
            <div class="flex items-center gap-2 text-xs font-medium text-slate-500">
                ...
                <span>/</span>
                <span class="text-slate-900 font-semibold capitalize">{{ $title ?? 'Dashboard' }}</span>
            </div>
            ...
            <div class="text-xs text-slate-500 font-medium">
                ...
                <span class="text-slate-700 font-semibold">Kelompok 05</span>
            </div>
