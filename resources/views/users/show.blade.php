    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-2">
            ...
            <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengguna {{ $user->name }}?')" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn-danger-soft text-xs py-2 px-3" aria-label="Hapus akun pengguna {{ $user->name }}">
    ...
    <div class="max-w-3xl mx-auto">
        <div class="lms-card p-6 sm:p-8">
            ...
            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 pb-6 border-b border-slate-100">
                <div class="w-16 h-16 rounded-lg bg-slate-100 border border-slate-200 text-slate-900 flex items-center justify-center font-bold text-xl flex-shrink-0" aria-hidden="true">
                ...
                <div class="text-center sm:text-left flex-1">
                    <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-1.5">
                        ...
                        @if ($user->role === 'admin')
                            <span class="badge-coral text-[11px]">Administrator</span>
                    ...
                    <p class="text-sm font-mono text-slate-600 mb-1 font-normal">{{ $user->email }}</p>
                    <p class="text-xs text-slate-600 font-normal">
                        NIM / NIP: <strong class="text-slate-900 font-mono font-semibold">{{ $user->nim_nip ?? 'Tidak ada' }}</strong>
                    </p>
                </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 text-xs text-slate-700 font-normal">
                <div class="p-4 rounded-lg bg-slate-50 border border-slate-200">
                    <span class="text-slate-600 block mb-1 font-semibold uppercase tracking-wider text-[11px]">Status Verifikasi Email</span>
                ...
                <div class="p-4 rounded-lg bg-slate-50 border border-slate-200">
                    <span class="text-slate-600 block mb-1 font-semibold uppercase tracking-wider text-[11px]">Terdaftar Sejak</span>
